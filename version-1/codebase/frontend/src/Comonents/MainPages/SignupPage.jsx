...
const SignupPage = () => {
  ...
  const validate = () => {
    ...
    //   let inValidMobile = "*Mobile number should be 10 digits"

      
    //   // if(Object.keys(formErrors).length > 0 && formErrors?.email !== inValidEmail && formErrors?.mobileNumber !== inValidMobile) {
    //   //   // formErrors.allError = '*All fields are required';
    //   //   setFormDataError({
    //   //     'allError': '*All fields are required'
    //   //   });
    //   // } 
    //   // else if(Object.keys(formErrors).length > 0 && formErrors?.email === inValidEmail && formErrors?.mobileNumber === inValidMobile){
    //   //   setFormDataError({
    //   //     email: formErrors?.email,
    //   //     mobileNumber: formErrors?.mobileNumber
    //   //   });
    //   // } 
    //   // else if(!isDobValid) {
    //   //   setFormDataError({
    //   //     "dob": "*User must be 16 years or older."
    //   //   })
    //   // }

    //   if (!isDobValid && Object.keys(formErrors).length === 1) {
    //     setFormDataError({
    //       dob: "*User must be 16 years or older."
    //     });
    //   } else if (
    ...
    //     formErrors?.mobileNumber !== inValidMobile
    //   ) {
    //     setFormDataError({
    //       allError: "*All fields are required"
    //     });
    ...
    if (step === 1) {
      ...
      const inValidMobile = "*Mobile number should be 10 digits";
      ...
      const emptyFieldsCount = Object.values(formErrors).filter((error) => error.includes("required")).length;
      const invalidFields = Object.values(formErrors).filter(
        (error) => error === inValidEmail || error === inValidMobile || error.includes("16 years")
      );
    
      // 1. Show "*All fields are required*" if more than one field is empty.
      if (emptyFieldsCount > 1) {
        setFormDataError({ allError: "*All fields are required" });
      } 
      // 2. If all fields are filled but there are invalid entries, show only the specific invalid field errors.
      else if (emptyFieldsCount === 0 && invalidFields.length > 0) {
        let validationErrors = {};
        if (formErrors.email === inValidEmail) validationErrors.email = inValidEmail;
        if (formErrors.mobileNumber === inValidMobile) validationErrors.mobileNumber = inValidMobile;
        if (formErrors.dob === "*User must be 16 years or older.") validationErrors.dob = formErrors.dob;
        setFormDataError(validationErrors);
      } 
      // 3. If exactly one field is empty, show the specific error for that field.
      else if (emptyFieldsCount === 1) {
        setFormDataError(formErrors);
      }
    ...
    if(isValid) {
      ...
      try {
        ...
        if(finalSignUpResult) {
          ...
          localStorage.setItem("token", finalSignUpResult.token);
          // navigate('/login');
